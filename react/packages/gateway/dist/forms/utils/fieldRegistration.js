 ...
 * });
 */
export var createFieldRegister = registerFn => {
  // Fast-path: already a wrapped controlled register — build handler without probing
  if (registerFn !== null && registerFn !== void 0 && registerFn.__isControlledRegister) {
  ...
  if (typeof registerFn === 'function') {
    try {
      ...
      var testResult = registerFn('__test__');

      // Check if the result has RHF's register signature
      if (testResult && typeof testResult === 'object' && 'name' in testResult && 'onChange' in testResult && 'onBlur' in testResult && 'ref' in testResult && typeof testResult.onChange === 'function') {
 ...
 */
export var createMockFormState = function createMockFormState() {
 ...
 */
export var createGutenbergRegister = setAttributes => {
  return (name, value) => {
    setAttributes({
      [name]: value
    });
 ...
 * });
 */
export var createControlledRegister = onChange => {
  var fn = (name, value) => {
    onChange(name, value);
  };
