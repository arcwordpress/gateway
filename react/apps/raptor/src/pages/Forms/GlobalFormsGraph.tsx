...
/**
 * Global forms graph showing all forms from all collections
 */
export function GlobalFormsGraph() {
  ...
  // Build nodes from all collections' forms - COMPUTED DIRECTLY
  const computedNodes: Node[] = []
  const computedEdges: Edge[] = []
  let yOffset = 0

  collections.forEach((collection) => {
    const forms = collection.form_list?.forms ?? []
    ...
    forms.forEach((form: any, idx: number) => {
      const nodeId = `form-${collection.id}-${form.form_key}`
      computedNodes.push({
        id: nodeId,
        data: {
          label: form.form_key,
  ...
  console.log('[GlobalFormsGraph] Computed nodes:', computedNodes.length, 'edges:', computedEdges.length)
  ...
  useEffect(() => {
    console.log('[GlobalFormsGraph] useEffect called, syncing computed nodes')
