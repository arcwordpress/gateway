    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">
            Each package becomes a WordPress admin menu with a unique URL
          </p>
        </div>
      ...
      {isError && (
        <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm">
          Failed to load packages. Check that the Gateway REST API is reachable.
        </div>
      )}
      ...
      {!isLoading && !isError && packages.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-zinc-500 text-sm mb-4">No packages yet</p>
          <Link
            to={'/packages/create' as never}
        ...
        <div className="space-y-2">
          {packages.map((pkg) => (
            <Link
              ...
              to={`/packages/${pkg.package_key}/edit` as any}
